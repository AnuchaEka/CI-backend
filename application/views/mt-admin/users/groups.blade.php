                                    ...
                                    <div class="pull-right">
                                        ...
                                        <tbody>
                                            ...
                                            <tr >
                                                <td>
                                                    <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                                        <input type="checkbox" name="del[]" class="checkboxes" value="{{$val->groupusers_id}}" />
                                                        <span></span>
                                                    </label>
                                                </td>
                                                <td>
                                                    <a href="{{base_url('mt-admin/'.$uri->segment(2).'/edit/'.$val->groupusers_id)}}"> {{$val->groupusers_name}} </a> 
                                                  
                                                  </td>
                                               <td>
                                                    <a href="@if(empty($val->groupusers_url) or $val->groupusers_url=='#') {{'javascript:;'}} @else {{base_url('mt-admin/'.$val->groupusers_url)}} @endif">{{$val->groupusers_url}}</a>
                                                </td>
                                                
                                                <td>
                                                 @if($val->active==0)     
                                                <a href="{{base_url('mt-admin/'.$uri->segment(2).'/status/'.$val->groupusers_id.'?status=1')}}" ><i class="fa fa-check  fa-2x text-success"></i></a>
                                                @else
                                                <a href="{{base_url('mt-admin/'.$uri->segment(2).'/status/'.$val->groupusers_id.'?status=0')}}" ><i class="fa fa-times  fa-2x text-danger"></i></a>
                                                @endif
